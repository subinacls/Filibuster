...
class jsonlogger():
	...
	def json_read(self):
		try:
			with open(str(consultant)+'-'+str(location)+'-'+dft+'.json', "r") as __builtin__.f: # open a file as named variable
	...
	def json_write(self):
		try:
			with open(str(consultant)+'-'+str(location)+'-'+str(dft)+'.json', "w") as __builtin__.f: # open a file as named variable
